...
open class ML_ContentView: UIView {
    ...
    fileprivate(set) lazy var collectionView: CustomGestureCollectionView = {
        ...
        collection.register(UICollectionViewCell.self, forCellWithReuseIdentifier: ML_ContentView.cellId)
        return collection
    }()
...
extension ML_ContentView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    final public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ML_ContentView.cellId, for: indexPath)
...
extension ML_ContentView: UIScrollViewDelegate {
    // update UI
...
public protocol ContentViewDelegate: class {
    ...
    /// 必须提供的属性
    var segmentView: ML_ScrollSegmentView { get }
}
